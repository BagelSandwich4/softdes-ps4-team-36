...
def make_results_plot(path, engine_type):
    ...
    plt.ylabel("Output of Engine (hp)")
    plt.title(f"Force applied to Accelarator vs. Output of Engine")
    plt.show()
```
...
```{python}
make_results_plot("data/test_design_2.csv",'vibranium')
```
...
```{python}
make_results_plot("data/test_design_3.csv", "flux capacitor")
```
...
```{python}
make_results_plot("data/test_design_1.csv", "palladium")
```
...
```{python}
make_results_plot("data/test_design_4.csv", "graphene")
```
